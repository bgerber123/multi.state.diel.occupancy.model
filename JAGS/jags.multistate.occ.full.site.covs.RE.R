    model{ 
    # Priors
      #Priors for mean population level intercepts and effects
      mu.alpha.day~dnorm(0,0.5)
      mu.alpha.night~dnorm(0,0.5)
      mu.alpha.ND~dnorm(0,0.5)
      mu.beta.day~dnorm(0,0.5)
      mu.beta.night~dnorm(0,0.5)
      mu.beta.ND.D~dnorm(0,0.5)
      mu.beta.ND.N~dnorm(0,0.5)
      mu.beta.ND.ND~dnorm(0,0.5)
      #Priors for the variance of the population level effects
      s2.alpha.day~dgamma(1,1)
      s2.alpha.night~dgamma(1,1)
      s2.alpha.ND~dgamma(1,1)
      s2.beta.day~dgamma(1,1)
      s2.beta.night~dgamma(1,1)
      s2.beta.ND.D~dgamma(1,1)
      s2.beta.ND.N~dgamma(1,1)
      s2.beta.ND.ND~dgamma(1,1)
      #inverse of variance
      tau.alpha.day<-1/s2.alpha.day
      tau.alpha.night<-1/s2.alpha.night
      tau.alpha.ND<-1/s2.alpha.ND
      tau.beta.day<-1/s2.beta.day
      tau.beta.night<-1/s2.beta.night
      tau.beta.ND.D<-1/s2.beta.ND.D
      tau.beta.ND.N<-1/s2.beta.ND.N
      tau.beta.ND.ND<-1/s2.beta.ND.ND
      #priors for fixed slopes
      alpha1.1~dlogis(0,1)
      alpha2.1~dlogis(0,1)
      alpha3.1~dlogis(0,1)
    # Define state vector
    for (t in 1:T){
      #These the population-level random effects for the intercepts. Slopes are fixed effects
      alpha1[t]~dnorm(mu.alpha.day, tau.alpha.day)
      alpha2[t]~dnorm(mu.alpha.night, tau.alpha.night)
      alpha3[t]~dnorm(mu.alpha.ND, tau.alpha.ND)
    for (i in 1:N[t]){
      phi[i,1,t] <- 1
      phi[i,2,t] <- exp(alpha1[t]+alpha1.1*cov1[i,t])
      phi[i,3,t] <- exp(alpha2[t]+alpha2.1*cov2[i,t])
      phi[i,4,t] <- exp(alpha1[t]+alpha1.1*cov1[i,t]+alpha2[t]+alpha2.1*cov2[i,t]+alpha3[t]+alpha3.1*cov3[i,t])
      PSI[i,1,t] <- phi[i,1,t]/sum(phi[i,,t])
      PSI[i,2,t] <- phi[i,2,t]/sum(phi[i,,t])
      PSI[i,3,t] <- phi[i,3,t]/sum(phi[i,,t])
      PSI[i,4,t] <- phi[i,4,t]/sum(phi[i,,t])
      #Mean occurence probabilities
      phiM[i,1,t] <- 1
      phiM[i,2,t] <- exp(mu.alpha.day+alpha1.1*cov1[i,t])
      phiM[i,3,t] <- exp(mu.alpha.night+alpha2.1*cov2[i,t])
      phiM[i,4,t] <- exp(mu.alpha.day+alpha1.1*cov1[i,t]+mu.alpha.night+alpha2.1*cov2[i,t]+mu.alpha.ND+alpha3.1*cov3[i,t])
      PSIM[i,1,t] <- phiM[i,1,t]/sum(phiM[i,,t])
      PSIM[i,2,t] <- phiM[i,2,t]/sum(phiM[i,,t])
      PSIM[i,3,t] <- phiM[i,3,t]/sum(phiM[i,,t])
      PSIM[i,4,t] <- phiM[i,4,t]/sum(phiM[i,,t])
      }
    }
    # Define observation matrix
    # Order of indices: true state, time, observed state, survey
    for (t in 1:T){
      beta1[t]~dnorm(mu.beta.day, tau.beta.day)
      beta2[t]~dnorm(mu.beta.night, tau.beta.night)
      beta3[t]~dnorm(mu.beta.ND.D, tau.beta.ND.D)
      beta4[t]~dnorm(mu.beta.ND.N, tau.beta.ND.N)
      beta5[t]~dnorm(mu.beta.ND.ND, tau.beta.ND.ND)
    for (j in 1:K){
    q[1,j,1,t] <- 1
    q[1,j,2,t] <- 0
    q[1,j,3,t] <- 0
    q[1,j,4,t] <- 0
    q[2,j,1,t] <- 1
    q[2,j,2,t] <- exp(beta1[t])
    q[2,j,3,t] <- 0
    q[2,j,4,t] <- 0
    q[3,j,1,t] <- 1
    q[3,j,2,t] <- 0
    q[3,j,3,t] <- exp(beta2[t])
    q[3,j,4,t] <- 0
    q[4,j,1,t] <- 1
    q[4,j,2,t] <- exp(beta3[t])
    q[4,j,3,t] <- exp(beta4[t])
    q[4,j,4,t] <- exp(beta3[t] + beta4[t] + beta5[t])
    }
      }
    # State-space likelihood
    # State equation: model of true states (z)
    for (t in 1:T){
    for (i in 1:N[t]){
     z[i,t] ~ dcat(phi[i,,t])
    }
    }
    # Observation equation
    for (t in 1:T){
    for (i in 1:N[t]){
       for (j in 1:K){ 
        y[i,j,t] ~ dcat(q[z[i,t],j,,t])
       } #j
    } #i
    }#t
#Derive outputs      
for (t in 1:T){      
      psiDay[t] <- PSI[1,2,t]  
      psiNight[t] <- PSI[1,3,t]
      psiND[t] <- PSI[1,4,t]
      #Derive detection parameters for occasion 1
      pDay[t] <- q[2,1,2,t]/sum(q[2,1,,t])
      pNight[t] <- q[3,1,3,t]/sum(q[3,1,,t])
      pND.ND[t] <- q[4,1,4,t]/sum(q[4,1,,t])
      pND.N[t] <- q[4,1,3,t]/sum(q[4,1,,t])
      pND.D[t] <- q[4,1,2,t]/sum(q[4,1,,t])
      pND.0[t] <- q[4,1,1,t]/sum(q[4,1,,t])      
}
}