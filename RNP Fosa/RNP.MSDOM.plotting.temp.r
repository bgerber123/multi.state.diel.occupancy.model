...
#for first fit
load("RNP Fosa/RNP Fit Original/M3.red.out")
...
for (i in 1:length(x.pred)){
  psiDay <-   plogis(alpha1+alpha2*x.pred.scaled[i])
  psiNight <-  plogis(alpha3+alpha4*x.pred.scaled[i])
  
  PSI2 <- psiDay*(1-psiNight)
  PSI3 <- psiNight*(1-psiDay)
  PSI4 <- psiDay*psiNight
  PSI1 <- (1-psiDay)*(1-psiNight)  #1-PSI[i,2]-PSI[i,3]-PSI[i,4]
  psi.state.mcmc[i,,1]     <- PSI1
  psi.state.mcmc[i,,2]     <- PSI2
  psi.state.mcmc[i,,3]     <- PSI3
  psi.state.mcmc[i,,4]     <- PSI4
